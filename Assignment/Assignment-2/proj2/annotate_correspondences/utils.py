...
def load_corr_pkl_file(corr_fpath):
    """Load ground truth correspondences from a .pkl or .mat file."""
    ext = os.path.splitext(corr_fpath)[1].lower()
    if ext == '.mat':
        d = sio.loadmat(corr_fpath)
    else:
        with open(corr_fpath, 'rb') as f:
            d = pickle.load(f, encoding='latin1')

    x1 = np.asarray(d['x1']).squeeze()
    y1 = np.asarray(d['y1']).squeeze()
    x2 = np.asarray(d['x2']).squeeze()
    y2 = np.asarray(d['y2']).squeeze()
